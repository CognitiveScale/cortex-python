...
class TestRun(unittest.TestCase):
    ...
    def test_make_remote_run(self):
        exp = Experiment(document=self.cortex.experiments.get_experiment(self.RUN_EXP_NAME), client=self.cortex.experiments)
        r = exp.start_run()
    ...
    def test_get_run(self):
        ...
        uri = self.cortex.experiments.URIs['run'].format(experimentName=self.RUN_EXP_NAME, projectId=PROJECT, runId=self.RUN_ID)
        returns = {'_id': '63cfb10ffe65fb07bf8a94b9', '_projectId': 'prajna-test', 'runId': 'run_01', 'experimentName': 'praj-op-gc_dtree_exp', 'params': {'category': 'Decision Tree', 'version': 1, 'SourceData': 'Upstream Server Data'}, 'metrics': {'accuracy': 0.68}, 'meta': {'algo': 'DecisionTreeClassifier'}, '_createdAt': '2023-01-24T10:21:03.120Z', '_updatedAt': '2023-01-24T10:21:04.497Z', 'artifacts': {'model': 'experiments/praj-op-gc_dtree_exp/run_01/artifacts/model'}}
    ...
    def test_list_runs(self):
        uri = self.cortex.experiments.URIs['runs'].format(experimentName=self.RUN_EXP_NAME, projectId=PROJECT)
        local_url = self.cortex.experiments._serviceconnector._construct_url(uri)
        returns = {"runs": {'_id': '63cfb10ffe65fb07bf8a94b9', '_projectId': 'prajna-test', 'runId': 'run_01', 'experimentName': 'praj-op-gc_dtree_exp', 'params': {'category': 'Decision Tree', 'version': 1, 'SourceData': 'Upstream Server Data'}, 'metrics': {'accuracy': 0.68}, 'meta': {'algo': 'DecisionTreeClassifier'}, '_createdAt': '2023-01-24T10:21:03.120Z', '_updatedAt': '2023-01-24T10:21:04.497Z', 'artifacts': {'model': 'experiments/praj-op-gc_dtree_exp/run_01/artifacts/model'}}}
    ...
    def test_update_run(self):
        ...
        body = {"success": True}
        Entry.single_register(Entry.PUT,
                              local_url,
                              status=200,
                              body=json.dumps(body))

        ret = self.cortex.experiments.update_run(experiment_name=self.RUN_EXP_NAME, run_id=self.RUN_ID, meta={"blah": 1})
    ...
    def test_delete_run(self):
        ...
        body = {"success": True}
        Entry.single_register(Entry.DELETE,
                              local_url,
                              status=200,
                              body=json.dumps(body))
